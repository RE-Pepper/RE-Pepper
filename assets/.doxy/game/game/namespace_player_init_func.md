

# Namespace PlayerInitFunc



[**Namespace List**](namespaces.md) **>** [**PlayerInitFunc**](namespace_player_init_func.md)










































## Public Functions

| Type | Name |
| ---: | :--- |
|  void \* | [**fn\_00260620**](#function-fn_00260620) () <br> |
|  const [**PlayerAnimInfo**](struct_player_anim_info.md) \* | [**getAnimInfo**](#function-getaniminfo) () <br> |
|  const [**PlayerModelInfo**](struct_player_model_info.md) \* | [**getModelInfo**](#function-getmodelinfo) () <br> |




























## Public Functions Documentation




### function fn\_00260620 

```C++
void * PlayerInitFunc::fn_00260620 () 
```




<hr>



### function getAnimInfo 

```C++
const PlayerAnimInfo * PlayerInitFunc::getAnimInfo () 
```




<hr>



### function getModelInfo 

```C++
const PlayerModelInfo * PlayerInitFunc::getModelInfo () 
```




<hr>

------------------------------
The documentation for this class was generated from the following file `Game/include/Player/PlayerInitFunc.h`

