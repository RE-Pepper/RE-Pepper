...
def getVerDirEx(ver): # Get directory for version
    return getDataDir() / "ver" / ver
def getVerDir(): # Get directory for version
    return getVerDirEx(getVersion())
...
def getBinAxfFileEx(v):
    return getVerDirEx(v) / "game.axf"
def getBinAxfFile():
    return getVerDir() / "game.axf"
def getBinCodeFileEx(v):
    return getVerDirEx(v) / "code.bin"
def getBinCodeFile():
    return getVerDir() / "code.bin"

def getBinFile():
    if getBinAxfFile().exists():
        return getBinAxfFile()
    else:
        return getBinCodeFile()
def getBinFileEx(v):
    if getBinAxfFileEx(v).exists():
        return getBinAxfFileEx(v)
    else:
        return getBinCodeFileEx(v)
